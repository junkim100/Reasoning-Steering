...
def scoring(
    ...
    custom_paths: dict[str, list[str]] = None,
    overwrite=False,
    do_printing=False,
):
    ...
    for behavior in behaviors:
        ...
        for file in open_ended_results:
            ...
            with open(os.path.join(copy_dir, os.path.basename(file)), "w") as f:
                ...
                for d in tqdm(data):
                    system_prompt, user_prompt = make_prompts(
                        d["question"], d["model_output"], behavior
                    )
                    ...
                    try:
                        ...
                        print(
                            f"Error scoring. Prompt: {user_prompt}, Response: {score}"
                        )
                        continue
...
if __name__ == "__main__":
    scoring(
        ...
        custom_paths={
            "sycophancy": [
                "results/sycophancy-subtraction/results_layer=0_multiplier=-1.0_behavior=sycophancy_type=open_ended_use_base_model=False_model_size=8B.json"
            ]
        },
        overwrite=True,
        do_printing=True,
